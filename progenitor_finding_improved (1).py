...
# use scipy.interp instead of a loop to increase the speed
...
x= np.linspace(0, 3, num=41, endpoint=False)
for i in x:
...
fMin=interp1d(x,mins)
fMax=interp1d(x,maxes)
...
plt.plot(x,fMin(x), label='minf')
plt.plot(x,fMax(x), label='maxf')
plt.plot(growth_curve[0], growth_curve[2], "#0F7969", label='max')
plt.plot(growth_curve[0], growth_curve[1], "#E71E06", label="ZFOURGE")
plt.plot(growth_curve[0], growth_curve[3], "#FF7969", label='min')
